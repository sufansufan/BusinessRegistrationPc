...
<template>
  <div class="campus-management-box">
              ...
              <span slot-scope="{ node, data }" class="campus-tree-node">
                ...
                <span class="campus-tree-btn">
                  <div v-if="node.label !== '总部'">
                    <el-button type="text" size="mini" @click="() => deleteCampus(data)"> <i class="el-icon-delete tree-icon"/> </el-button>
                  </div>
                  <div v-if="node.label !== '总部'">
                    <el-button type="text" size="mini" @click="() => modifyCampus(data,'tree')">  <i class="el-icon-edit tree-icon"/></el-button>
                  </div>
                  <div>
        ...
        <el-col :span="18">
          <el-table v-loading="loading" :data="dataList" stripe border style="width: 100%">
            ...
            <el-table-column label="操作" width="250" fixed="right">
              <template slot-scope="scope">
                <el-button :disabled="!scope.row.whetherEdit" type="text" @click="modifyCampus(scope.row, 'table')">编辑</el-button>
                <el-button type="text" @click="modifyStatus(scope.row)">{{ scope.row.status === true ? '停用' : '启用' }}</el-button>
                <el-button :disabled="!scope.row.whetherDelete" type="text" @click="deleteCampus(scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
    ...
    <Dialog ref="dialog">
      <el-form ref="campusForm" :model="campusForm" :rules="campusRules" :key="key" class="campus-form">
