  ...
  <div>
    <div class="select-tools">
      <div>
        ...
        <div class="inline">
          <el-button type="primary" icon="el-icon-plus" @click="$router.push({name: 'AddCouponer'})">新增</el-button>
    ...
    <comm-table v-loading="loading" :data="dataList" :columns="columns" :count="count">
      <div slot-scope="scope">
        <el-button type="text" @click="$router.push({path: '/subject/discountCoupon/check/' + scope.row.id})">查看</el-button>
      </div>
    </comm-table>
  </div>
</template>
...
import { getCouponGrantList } from '@/api/subject/discountCoupon'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { label: '名称', prop: 'name' },
        { label: '优惠券模板', prop: 'couponName', width: '100' },
        { label: '数量', prop: 'count', type: 'num' },
  ...
  methods: {
    fetchData(type) {
      ...
      this.BFD(getCouponGrantList(params).then(res => {
