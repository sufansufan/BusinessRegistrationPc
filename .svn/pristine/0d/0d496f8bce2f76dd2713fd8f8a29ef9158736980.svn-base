      ...
      this.$refs.userManagementForm.validate().then(() => {
        ...
        addAndEditSysUser({
          ...
          role: role.map((item, index) => {
            item.campusIds = defRole[index].campusIds
            return item
          })
        }).then(() => {
